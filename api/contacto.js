...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta GameFix - gamefix.com.es | ${modelo||'Consola'}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#141418"><h2 style="color:#B4130C">Nueva consulta desde GameFix</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Consola o mando:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#57646F">Enviado desde el formulario de gamefix.com.es</p></div>`;
    const mime=[`From: GameFix <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
